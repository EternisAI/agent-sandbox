...
17. **Never infer 52-week high/low or all-time high from a rolling `get_aggs` window.** A short lookback (e.g. 20 days) will silently miss earlier peaks and produce wrong ATH claims. Use the `finnhub` skill instead: `client.company_basic_financials("ASTS", "all")` returns `metric["52WeekHigh"]` and `metric["52WeekHighDate"]` directly (verified working, e.g. ASTS → `52WeekHigh: 133.86, 52WeekHighDate: 2026-05-28`). Only fall back to `get_aggs` if you need intraday precision or the ticker isn't covered by Finnhub.
